...
func (user *User) finishInsert(inviterId int) {
	if operation_setting.GetQuotaSetting().AutoSubscribeFreePlansForNewUser {
		if err := AutoSubscribeFreePlansForNewUser(user.Id); err != nil {
			common.SysError(fmt.Sprintf("failed to auto-subscribe new user %d to free plans: %v", user.Id, err))
		}
	}
...
func (user *User) FinalizeOAuthUserCreation(inviterId int) {
	if operation_setting.GetQuotaSetting().AutoSubscribeFreePlansForNewUser {
		if err := AutoSubscribeFreePlansForNewUser(user.Id); err != nil {
			common.SysError(fmt.Sprintf("failed to auto-subscribe new OAuth user %d to free plans: %v", user.Id, err))
		}
	}
