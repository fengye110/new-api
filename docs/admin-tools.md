...
- 已注销用户的行操作仅保留删除按钮；删除会经过现有确认流程，并对软删除记录执行永久删除。
- Root User 可在用户行操作菜单中将普通用户或管理员提升为 Root User；操作会要求确认并立即刷新用户与令牌缓存。
- Root User 可禁用、永久删除或降级其他 Root User；系统禁止对当前登录的 Root User 执行这些操作。
- 用户列表会高亮角色：Root User 使用琥珀色背景，管理员使用天蓝色背景；默认与经典前端均兼容深色和浅色主题。
...
- 入口：`System Settings` -> `Billing` -> `Quota Settings` -> “New user subscription visibility rules”。可为多个邮箱设置默认订阅可见权限；匹配邮箱的新注册用户会在创建事务中自动获得对应权限，未选择权限组时使用默认可见权限组。
- 用户停用订阅后，该订阅会立即退出扣费与分组选择；系统会使用其他有效订阅中 `Sort Order` 最高套餐的升级分组，若不存在则按该订阅的降级设置恢复用户分组。重新启用时也会重新选择最高优先级升级分组。
